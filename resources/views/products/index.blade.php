...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
  ...
  <div class="flex items-center gap-2 flex-wrap">
    ...
    <button type="button" class="btn-outline-sm" onclick="document.getElementById('importModal').showModal()" title="Import Update Harga">
...
<div class="mb-4">
  <x-search-bar
    ...
    :filters="[
      ['name' => 'category_id', 'label' => 'Semua Kategori', 'type' => 'select',
       'options' => $categories->pluck('name', 'id')->toArray()],
      ['name' => 'is_active', 'label' => 'Semua Status', 'type' => 'select',
       'options' => ['1' => 'Aktif', '0' => 'Nonaktif']],
      ['name' => 'low_stock', 'label' => 'Stok', 'type' => 'select',
       'options' => ['1' => 'Stok Kritis']],
    ]"
  />
</div>
        ...
        <tr class="group">
          <td>
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-slate-50 border border-slate-200 flex items-center justify-center text-lg">
                @if ($product->image)
          ...
          <td class="font-semibold text-slate-900">Rp {{ number_format($product->sell_price, 0, ',', '.') }}</td>
          <td>
            <span class="font-bold {{ $product->is_low_stock ? 'text-red-600' : 'text-slate-900' }}">{{ $product->stock_qty }}</span>
            @if ($product->is_low_stock)
              <div class="text-[10px] text-red-500">⚠ min {{ $product->stock_min }}</div>
            @endif
          </td>
          <td>
        ...
        <tr>
          <td colspan="7">
            <div class="py-16 text-center text-slate-400">
              ...
              <span class="text-sm">
                @if (request('search'))
                  Tidak ada produk yang cocok dengan "<strong class="text-slate-600">{{ request('search') }}</strong>"
                @else
...
<dialog id="importModal" class="rounded-2xl shadow-2xl border-0 p-0 w-full max-w-md backdrop:bg-slate-900/50 backdrop:backdrop-blur-sm">
  <form method="POST" action="{{ route('products.import-prices') }}" enctype="multipart/form-data">
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
      ...
      <button type="button" class="text-slate-400 hover:text-slate-600 text-xl leading-none" onclick="document.getElementById('importModal').close()">&times;</button>
    </div>
    <div class="p-6">
      <p class="text-sm text-slate-500 mb-5 leading-relaxed">
        Unggah file Excel (.xlsx) yang telah diunduh dan isi kolom <strong class="text-slate-700">"Harga Baru"</strong>. Kosongkan kolom jika harga tidak berubah.
      </p>
...
<dialog id="stockModal{{ $product->id }}"
  ...
  <form method="POST" action="{{ route('products.adjust-stock', $product) }}"
    onsubmit="return validateStockModal({{ $product->id }}, {{ $product->stock_qty }})">
    @csrf
    ...
    <div class="p-5">
      ...
      <div class="flex items-center justify-between mb-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
        ...
        <span class="text-base font-bold font-mono {{ $product->is_low_stock ? 'text-red-600' : 'text-teal-600' }}">
          {{ $product->stock_qty }}
        </span>
      </div>
      ...
      <div class="flex gap-2 mb-4">
        <button type="button" id="mBtnAdd{{ $product->id }}"
          onclick="setModalType({{ $product->id }}, {{ $product->stock_qty }}, 'add')"
        ...
        <button type="button" id="mBtnRed{{ $product->id }}"
          onclick="setModalType({{ $product->id }}, {{ $product->stock_qty }}, 'reduce')"
          class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border-2 text-sm font-semibold transition-all
      ...
      <div class="flex items-center gap-2 mb-3">
        <button type="button"
          onclick="changeModalQty({{ $product->id }}, {{ $product->stock_qty }}, -1)"
        ...
        <input type="number" id="mQty{{ $product->id }}" value="1" min="1"
          oninput="refreshModalPreview({{ $product->id }}, {{ $product->stock_qty }})"
      ...
      <div class="flex gap-2 mb-4">
        ...
        <button type="button" onclick="setModalQty({{ $product->id }}, {{ $product->stock_qty }}, {{ $p }})"
...
function setModalType(pid, currentStock, type) {
  ...
  if (type === 'add') {
    btnAdd.className = btnAdd.className.replace(/border-slate-200 bg-white text-slate-400/, 'border-teal-600 bg-teal-600 text-white');
    btnRed.className = btnRed.className.replace(/border-teal-600 bg-teal-600 text-white|border-red-600 bg-red-600 text-white/, 'border-slate-200 bg-white text-slate-400');
    btnAdd.className = btnAdd.className.replace(/border-slate-200 bg-white text-slate-400/, 'border-teal-600 bg-teal-600 text-white');
  } else {
    btnRed.className = btnRed.className.replace(/border-slate-200 bg-white text-slate-400/, 'border-red-600 bg-red-600 text-white');
    btnAdd.className = btnAdd.className.replace(/border-teal-600 bg-teal-600 text-white/, 'border-slate-200 bg-white text-slate-400');
  }
